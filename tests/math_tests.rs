...
fn test_limit_polynomial() {
    ...
    assert!((result - 4.0).abs() < 1e-6); // Limit f(x) saat x mendekati 2 adalah 4
}
...
fn test_limit_trigonometric() {
    ...
    assert!((result - (-1.0 / 6.0)).abs() < 1e-6); // Limit f(x) saat x mendekati 0 adalah -1/6
}
...
fn test_limit_logarithmic() {
    ...
    assert!((result - 0.0).abs() < 1e-6); // Limit f(x) saat x mendekati 1 adalah 0
}
...
fn test_limit_exponential() {
    ...
    let result = limit(f, 0.0, 1e-6).unwrap();
    assert!((result - 0.5).abs() < 1e-6); // Limit f(x) saat x mendekati 0 adalah 0.5
}
